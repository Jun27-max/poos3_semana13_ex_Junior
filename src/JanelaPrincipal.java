...
public class JanelaPrincipal extends JFrame implements ActionListener {
	...
	private void salvarJogo() {
		sz = new SerializadorObj(new File("JogoDado.ser"));
		sz.serializar(jogo);
		jogo.setStatus(StatusEnum.ANDAMENTO);
		areaJogo.append(String.format("Jogo salvo\n"));
		// JOptionPane.showMessageDialog(areaJogo, "Jogo Salvo");
	}

	private void recuperarJogo() {
		try {
			JogoDado j = (JogoDado) sz.desseralizar();
			if (j != null) {
				j.setStatus(StatusEnum.INICIADO);
				areaJogo.append(String.format("Jogo Reiniciado\n"));
			}
			areaJogo.append(String.format("Jogo não existe! \n"));
